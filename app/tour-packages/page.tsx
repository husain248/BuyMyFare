...
function page() {
  const packages = [
    ...
    },
    {
      ...
      location: "Jamaica",
      title: "Jamaica",
      description: "Nestled between lush mountains and stunning beaches, Jamaica boasts a vibrant island culture and some of the Caribbean’s most scenic coastlines. Whether you’re seeking an all-inclusive resort or an adults-only retreat, there’s a wide variety of accommodations to suit all types of travelers.",
      price: "$1175"
    },
    {
      ...
      location: "Cuba",
      title: "Cuba",
      description: "With an average of 330 sunny days per year and over 400 kilometers of unspoiled coastline, Cuba is an ideal destination for a peaceful beach escape. Relax on the famous white sands of Varadero Beach, dive into the crystal-clear waters of Playa Pilar’s coral reefs, or take a stroll along the tranquil shores of Playa Paraiso.",
      price: "$625"
    },
    {
    ...
    <div className="2xxl:min-h-100 lg:min-h-100 sm:min-h-100 min-h-75 text-center relative bg-no-repeat bg-center bg-cover before:absolute before:inset-0 before:bg-black/20" style={{ backgroundImage: "url('/assets/images/banner/bnr2.png')" }}>
        <div className="container relative z-1 table h-full">
            <div className="text-center py-15 table-cell align-middle 2xxl:h-125 lg:h-125 sm:h-100 h-75">
              ...
              fontSize: "32px",color: "#ffffff",}}>Customized Vacation Deals</span>
                <h1 className="2xxl:text-11xxl xl:text-10xl sm:text-8xl text-5xl mb-2.5 text-white">Tour Packages</h1>
            </div>
        </div>  
    </div>
    <section className='lg:pt-25 lg:pb-45 sm:py-25 py-18 overflow-hidden'>
      <div className='container-left max-w-[1330px]'>
        <div className="text-center mb-12">
          <h2 className="2xxl:text-10xl lg:text-8xl md:text-5xxl sm:text-4xl text-3xxl uppercase lg:mb-4 headline">Popular
Packages</h2></div>
  ...
  <div key={index} className="xl:col-span-6 col-span-12">
    <div className="relative z-1 p-2.5 h-full duration-200 bg-[#F4F4F4] rounded-2xl md:flex group">
      <div className="rounded-xxl xl:w-73.75 md:w-62.5 w-full xl:min-w-73.75 md:min-w-62.5 min-w-full md:h-full sm:h-62.5 h-50 relative overflow-hidden before:absolute before:top-0 before:-left-[75%] before:z-0 before:block before:w-1/2 before:h-full before:skew-x-[-25deg] before:bg-linear-(--img-hover-gradient) group-hover:before:animate-dzShine">
        <img src={pkg.img} alt="" className="size-full object-cover" />
        <span className="absolute top-5 left-5 rounded-3xl text-xs font-medium px-2.5 py-1.5 bg-green text-secondary">{pkg.location}</span>
      </div>
      <div className="sm:p-7 sm:pe-5 sm:pb-5 p-4 relative flex flex-col w-full">
        <div className="mb-2.5 flex-1">
          <h3 className="xl:text-2xl text-xl uppercase mb-2.5"><a href="tour-packages.html" className="link-hover">{pkg.title}</a></h3>
          <p className="text-sm line-clamp-4 mb-5">{pkg.description}</p>
        </div>
        <div className="flex items-center justify-between">
          <h4 className="text-2xxl flex flex-col items-start gap-1.5 font-semibold">
            <small className='text-sm w-full'>Starting from</small> {pkg.price}
          </h4>
